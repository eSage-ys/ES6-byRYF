...
setTimeout(function router() { 
  ...
  // todo 改造
  function getMakeDownContent (path) {
  ...
  menu.map(el => {
    var p = el.replace('#', './') + '.md';
    questList.push(getMakeDownContent(p));
  })

  Promise.all(questList).then(datas => {
    $(ditto.error_id).hide();
    ...
    // var data = '';
    datas.map(el => {
      $(ditto.content_id).append(marked(el));
      // data += el + '\n\r ------';
    });
    clearInterval(loading);
    $(ditto.loading_id).hide();

    // $(ditto.content_id).append(marked(data) + disqusCode);
  ...
  /* $.get(path, function(data) {
    $(ditto.error_id).hide();
    // $(ditto.content_id).append(marked(data) + disqusCode);
    $(ditto.content_id).append(marked(data));
    if ($(ditto.content_id + " h1").text() === ditto.document_title) {
      document.title = ditto.document_title;
    } else {
      document.title = $(ditto.content_id + " h1").text() + " - " + ditto.document_title;
